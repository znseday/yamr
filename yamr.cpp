...
void Yamr::Map()
{
    ...
    MY_DEBUG_ONLY( cout << "SectionsDataTemp.size() = " << SectionsDataTemp.size() << endl; )

    for (size_t i = 0; i < M; i++)
    {
        ts.emplace_back( new thread( [&m, i, &f, &SectionsDataTemp](size_t start, size_t end)
        {
            m.lock();
            MY_DEBUG_ONLY( cout << "thread " << i << ": start: " << start << "   end: " << end << endl; )
            ...
            {
                getline(f, line);
                MY_DEBUG_ONLY( cout << "thread " << i << ": line: " << line << endl; )
                ...
                MY_DEBUG_ONLY( cout << "thread " << i << ": curPos: " << curPos << endl; )
...
void Yamr::Reduce()
{
    ...
    vector<Reducer> reducers(R);

    for (size_t i = 0; i < R; i++)
    {
        ts.emplace_back( new thread( [this, &reducers](int i/*, Reducer &reducer*/)
        {
            for (const auto &s : DataForReducers[i])
            {
                //MY_DEBUG_ONLY( m.lock(); cout << s << endl; m.unlock();)
                reducers[i](s);
            }

        }, // end of lambda
        i  // lambda params
        //,reducers[i]  //  It doesnt' work as a parametr for lambda. Why???
                             )   );
    } // end for
